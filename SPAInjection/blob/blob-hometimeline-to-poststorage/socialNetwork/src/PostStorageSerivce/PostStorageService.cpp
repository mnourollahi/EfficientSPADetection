...
int main(int argc, char *argv[]) {
  ...
  int redis_port = 6379;
  std::string post_storage_addr = "127.0.0.1";
  int post_storage_port = 9090;

  // Initialize client pools
  ClientPool<RedisClient> redis_client_pool("post-storage-redis",
      redis_addr, redis_port, 0, 128, 1000);

  ClientPool<ThriftClient<PostStorageServiceClient>> post_client_pool(
      "post-storage-client", post_storage_addr, post_storage_port, 0, 128, 1000);
  ...
  TThreadedServer server (
      std::make_shared<PostStorageServiceProcessor>(
        ...
        &redis_client_pool, &post_client_pool)),
