...
class PostStorageHandler : public PostStorageServiceIf {
 public:
  ...
  void ReadUserTimeline(std::vector<Post> &_return, int64_t req_id, 
      int64_t user_id, const std::map<std::string, std::string> &carrier) override;
...
void ReadUserTimeline(
    ...
    // Core logic for reading user timeline
    TextMapReader reader(carrier);
    std::map<std::string, std::string> writer_text_map;
    TextMapWriter writer(writer_text_map);
    auto parent_span = opentracing::Tracer::Global()->Extract(reader);
    auto span = opentracing::Tracer::Global()->StartSpan(
        "ReadUserTimeline-postStorage", {opentracing::ChildOf(parent_span->get())});
    opentracing::Tracer::Global()->Inject(span->context(), writer);
    ...
    if (!collection) {
      ServiceException se;
      se.errorCode = ErrorCode::SE_MONGODB_ERROR;
      se.message = "Failed to get collection user-timeline from MongoDB";
      mongoc_client_pool_push(_mongodb_client_pool, mongodb_client);
      throw se;
    }
    ...
    auto find_span = opentracing::Tracer::Global()->StartSpan(
        "MongoFindUserTimeline-postStorage", {opentracing::ChildOf(&span->context())});
    mongoc_cursor_t* cursor = mongoc_collection_find_with_opts(
    ...
    if (!found) {
      ...
      if (mongoc_cursor_error(cursor, &error)) {
        ...
        mongoc_collection_destroy(collection);
        mongoc_client_pool_push(_mongodb_client_pool, mongodb_client);
        return;  // Timeline not found, return empty result
      }
    }

    auto timeline_json_char = bson_as_json(doc, nullptr);
    json timeline_json = json::parse(timeline_json_char);
    for (auto& item : timeline_json["posts"]) {
      Post new_post;
      new_post.post_id = item["post_id"];
      new_post.timestamp = item["timestamp"];
      _return.emplace_back(new_post);
    }
    bson_free(timeline_json_char);
