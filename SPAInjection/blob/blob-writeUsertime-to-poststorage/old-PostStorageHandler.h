...
void PostStorageHandler::StorePost(
  ...
  // Connect to MongoDB and insert post
  auto write_span = opentracing::Tracer::Global()->StartSpan(
      "WriteUserTimeline", {opentracing::ChildOf(&span->context())});
  mongoc_client_t *mongodb_client = mongoc_client_pool_pop(_mongodb_client_pool);
  if (!mongodb_client) {
    ServiceException se;  // Declaring ServiceException here
  ...
  if (!collection) {
    ServiceException se;  // Declaring ServiceException here
    se.errorCode = ErrorCode::SE_MONGODB_ERROR;
    se.message = "Failed to create collection from MongoDB";
  ...
  // Connect to Redis for timeline update
  memcached_st *memcached_client = memcached_create(nullptr);
  if (!memcached_client) {
    ServiceException se;  // Declaring ServiceException here
    se.errorCode = ErrorCode::SE_MEMCACHED_ERROR;
    se.message = "Failed to create Memcached client";
    throw se;
  }

  memcached_return_t memcached_rc = memcached_server_add(memcached_client, "localhost", 11211);
  ...
  // Check for existing posts in Redis timeline
  std::string key = std::to_string(post.creator.user_id);
  auto num_posts = memcached_get(memcached_client, key.c_str(), key.size(), nullptr, nullptr, &memcached_rc);
  if (memcached_rc == MEMCACHED_NOTFOUND) {
    LOG(info) << "User timeline not found in Redis, creating a new one.";
    std::multimap<std::string, std::string> value = { {std::to_string(post.timestamp), std::to_string(post.post_id)} };
    for (const auto &v : value) {
      memcached_rc = memcached_set(memcached_client, v.first.c_str(), v.first.size(), v.second.c_str(), v.second.size(), static_cast<time_t>(0), static_cast<uint32_t>(0));
      if (memcached_rc != MEMCACHED_SUCCESS) {
        LOG(error) << "Failed to update Redis for user timeline: " << memcached_strerror(memcached_client, memcached_rc);
        ServiceException se;  // Declaring ServiceException here
        se.errorCode = ErrorCode::SE_MEMCACHED_ERROR;
        se.message = memcached_strerror(memcached_client, memcached_rc);
        memcached_free(memcached_client);
    ...
    LOG(info) << "User timeline found in Redis, updating it.";
    std::multimap<std::string, std::string> value = { {std::to_string(post.timestamp), std::to_string(post.post_id)} };
    memcached_rc = memcached_set(memcached_client, key.c_str(), key.size(), value.begin()->second.c_str(), value.begin()->second.size(), static_cast<time_t>(0), static_cast<uint32_t>(0));
    if (memcached_rc != MEMCACHED_SUCCESS) {
      LOG(error) << "Failed to update Redis for user timeline: " << memcached_strerror(memcached_client, memcached_rc);
      ServiceException se;  // Declaring ServiceException here
      se.errorCode = ErrorCode::SE_MEMCACHED_ERROR;
    ...
    LOG(error) << "Redis error while fetching timeline: " << memcached_strerror(memcached_client, memcached_rc);
    ServiceException se;  // Declaring ServiceException here
    se.errorCode = ErrorCode::SE_MEMCACHED_ERROR;
    se.message = memcached_strerror(memcached_client, memcached_rc);
    memcached_free(memcached_client);
...
void PostStorageHandler::ReadPost(
  ...
  if (memcached_rc != MEMCACHED_SUCCESS) {
    ...
    se.errorCode = ErrorCode::SE_MEMCACHED_ERROR;
    se.message = memcached_strerror(memcached_client, memcached_rc);
    memcached_free(memcached_client);  // Free the client to avoid memory leaks
    throw se;
  }
...
void PostStorageHandler::ReadPosts(
  ...
  // If post_ids is empty, return immediately
  if (post_ids.empty()) {
    return;
  }

  // Ensure that there are no duplicated post_ids
