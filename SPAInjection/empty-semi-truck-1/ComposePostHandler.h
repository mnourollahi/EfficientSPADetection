//Separate Redis Commands with sync_commit(): We commit each call separately to simulate overhead.
//Error Handling for Replies and Threads:
//The cpp_redis::reply objects are declared and populated inside a try-catch block to handle Redis fetch errors gracefully.
//join() is called on each thread (upload_post_worker, upload_user_timeline_worker, and upload_home_timeline_worker) to ensure all components complete, and any thread errors are caught and logged.
...
class ComposePostHandler : public ComposePostServiceIf {
 ...
 private:
  ...
  void _ComposeAndUpload(int64_t req_id, const std::map<std::string, std::string> & carrier);
  ...
  void _UploadHomeTimelineHelper(int64_t req_id, int64_t post_id,
      int64_t user_id, int64_t timestamp,
      const std::vector<int64_t> & user_mentions, // Const reference to avoid copying
      const std::map<std::string, std::string> & carrier); // Const reference for carrier
...
void ComposePostHandler::_ComposeAndUpload(
  ...
  // Retrieve individual components from Redis separately to increase overhead
  ...
  // Create threads for different upload operations
  std::thread upload_post_worker(&ComposePostHandler::_UploadPostHelper,
                                 this, req_id, post, carrier);
		
  // Declare redis replies for later use
  ...
  // Try to get all replies, catch exceptions if any retrieval fails
  try {
  ...
  // Check Redis replies for errors
  ...
  // Compose the post with retrieved components
  Post post;
  post.req_id = req_id;
  post.text = f_text_reply.get().as_string();
  post.post_id = std::stoul(f_creator_reply.get().as_string());
  post.timestamp = duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count();
  post.post_type = static_cast<PostType::type>(std::stoi(post_type_reply.as_string()));
...
// Start the threads for async uploading
  std::thread upload_post_worker(&ComposePostHandler::_UploadPostHelper, this, req_id, post, carrier);
  std::thread upload_user_timeline_worker(&ComposePostHandler::_UploadUserTimelineHelper, this,
                                          req_id, post.post_id, post.creator.user_id, post.timestamp, carrier);

  // Directly pass required arguments to _UploadHomeTimelineHelper
  std::thread upload_home_timeline_worker(&ComposePostHandler::_UploadHomeTimelineHelper, this,
                                          req_id, post.post_id, post.creator.user_id, post.timestamp,
                                          std::cref(post.user_mentions), std::cref(carrier));																   
  ...
  // Error handling for each thread
  if (_user_timeline_teptr) {
    try {
      ...
      LOG(error) << "UserTimeline thread exited with exception: " << ex.what();
    }
  }
  if (_rabbitmq_teptr) {
    try {
      ...
      LOG(error) << "RabbitMQ thread exited with exception: " << ex.what();
    }
  }
  if (_post_storage_teptr) {
    try {
      ...
      LOG(error) << "PostStorage thread exited with exception: " << ex.what();
    }
  }
}
...
void ComposePostHandler::_UploadUserTimelineHelper(
    ...
    int64_t user_id,
    int64_t timestamp,
	const std::vector<int64_t> &user_mentions, // Updated to const reference

    const std::map<std::string, std::string> &carrier) { // Updated to const reference
  try{
