...
def create_isolation_forest_pipeline():
    return Pipeline([
        ('scaler', StandardScaler()),
...
def create_fuzzy_knn_pipeline():
    """
    Creates a fuzzy KNN pipeline where predictions include membership probabilities.
    """
    return Pipeline([
        ('scaler', StandardScaler()),
        ('model', KNeighborsClassifier(n_neighbors=5, weights='distance'))  # 'distance' weighting for fuzzy effect
    ])


def create_fuzzy_minibatch_kmeans_pipeline():
    """
    Creates a fuzzy MiniBatchKMeans pipeline providing soft clustering using distances.
    """
    return Pipeline([
        ('scaler', StandardScaler()),
        ('model', MiniBatchKMeans(n_clusters=2, random_state=42, batch_size=100))
...
def compute_trace_level_centrality(df_service):
    ...
    based on Sorce_Service and Target_Service pairs within that trace.
    """
...
if __name__ == "__main__":
    ...
    # df = df[df['send_syscall_count'] != 0].reset_index(drop=True)
    #
    ...
    # Predict
    # predictions = predict_anomalies(df_test, models, FEATURES)
    predictions = predict_clusters(df_test, models, FEATURES)
    # predictions = predict_anomalies(df_service_test, models, FEATURES)

    # Combine heuristics and ML results
    data = pd.concat([df_test, predictions], axis=1)
    # data = pd.concat([df_service_test, predictions], axis=1)
    print(data.columns)
    # data['Request_Intensity'] = data['Request_Intensity'].round(2)
    # data['betweenness'] = data['betweenness'].round(2)
    # data['degree'] = data['degree'].round(2)
    # data['closeness'] = data['closeness'].round(2)
    # data[('Blob', 'FuzzyMiniBatchKMeans_membership_0')] = data[('Blob', 'FuzzyMiniBatchKMeans_membership_0')].round(2)
    # data[('Blob', 'FuzzyMiniBatchKMeans_membership_1')] = data[('Blob', 'FuzzyMiniBatchKMeans_membership_1')].round(2)
    # data[('Est', 'FuzzyMiniBatchKMeans_membership_0')] = data[('Est', 'FuzzyMiniBatchKMeans_membership_0')].round(2)
    # data[('Est', 'FuzzyMiniBatchKMeans_membership_1')] = data[('Est', 'FuzzyMiniBatchKMeans_membership_1')].round(2)
    data.to_csv("data.csv", index=False)


    data['duplicate_count'] = data.groupby(['service_name', 'op_name', ('Est', 'MiniBatchKMeans'), ('Est', 'GaussianMixture'), ('Est', 'KNNAnomalyDetection')])[
        'service_name'].transform('count')

    # data['duplicate_count'] = data.groupby(['Source_Service', 'Target_Service', ('Blob', 'IsolationForest'), ('Blob', 'OneClassSVM')])[
    #     'Source_Service'].transform('count')

    # data['duplicate_count'] = \
    # data.groupby(['Source_Service', 'Target_Service', ('Blob', 'IsolationForest'), ('Blob', 'FuzzyMiniBatchKMeans_membership_0'),
    #      ('Blob', 'FuzzyMiniBatchKMeans_membership_1')])[
    #     'Source_Service'].transform('count')


    # Select unique rows with duplicate counts
    # filtered_df = data[
    #     ['service_name', 'op_name', ('Est', 'IsolationForest'),('Est', 'FuzzyMiniBatchKMeans_membership_0'),
    #          ('Est', 'FuzzyMiniBatchKMeans_membership_1'), 'duplicate_count']].drop_duplicates().reset_index(
    #     drop=True)
    filtered_df = data[
        ['service_name', 'op_name', ('Est', 'MiniBatchKMeans'), ('Est', 'GaussianMixture'), ('Est', 'KNNAnomalyDetection'),'duplicate_count']].drop_duplicates().reset_index(
        drop=True)

    # filtered_df = data[
    #     ['Source_Service', 'Target_Service', ('Blob', 'IsolationForest'), ('Blob', 'OneClassSVM'), 'duplicate_count']].drop_duplicates().reset_index(
    #     drop=True)

    # filtered_df = data[
    #     ['Source_Service', 'Target_Service', ('Blob', 'IsolationForest'), ('Blob', 'FuzzyMiniBatchKMeans_membership_0'),
    #      ('Blob', 'FuzzyMiniBatchKMeans_membership_1'), 'duplicate_count']].drop_duplicates().reset_index(
    #     drop=True)
